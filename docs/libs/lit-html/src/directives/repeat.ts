...
const createAndInsertPart = (containerPart: NodePart, beforePart?: NodePart): NodePart => {
...
const insertPartBefore = (containerPart: NodePart, part: NodePart, ref?: NodePart) => {
  const container = containerPart.startNode.parentNode as Node;
...
export const repeat = directive(
  <T>(items: Iterable<T>, keyFnOrTemplate: KeyFn<T> | ItemTemplate<T>, template?: ItemTemplate<T>): DirectiveFn => {
    let keyFn: KeyFn<T>;
...
) as <T>(items: Iterable<T>, keyFnOrTemplate: KeyFn<T> | ItemTemplate<T>, template?: ItemTemplate<T>) => DirectiveFn;
