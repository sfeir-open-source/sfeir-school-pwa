...
export class Login extends LitElement {
  constructor() {
    ...
    this.webauthService = new WebAuthService(this.peoplesService.getServerUrl());
    this.loading = true;
    this.query = '';
    this.signup = false;
    ...
    this.peoples = [];
    this.webAuthNHelper = new WebAuthNHelper(this.webauthService);
    ...
    if (action === 'signup') {
      this.peoplesService
        ...
        .then(data => {
          ...
          this.userLogged = data;
          this.authService.setSignedInUser(data);
          // TODO Séparer le flow webauthn du flow classique
          return data;
        })
        .then(data => {
          // We get the challenge from the server
          return this.webAuthNHelper.registration(data.username);
        })
        .then(async response => {
          console.log('Anwser', response);
          this.requestUpdate();
        });
    } else {
      this.peoplesService
